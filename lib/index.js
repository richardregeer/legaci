...
function createStartupDosBoxConfig(destination) {
  ...
  // TODO: Reading and writing to file will use to correct encoding. Do I really need to do this??
  // fs.createReadStream(result[0])
  //   .pipe(fs.createWriteStream(`${destination}/legaci-start.conf`))
  //   .end();
  ...
  // for (let i; i < startupConfig.length; i++) {
  //   const line = startupConfig[i];
  //   console.log(line);
  //   newStartupConfig.push(line);
  // }

  startupConfig.forEach((line) => {
    let newLine = line;
    if (line.indexOf('mount C ".."') > -1) {
      newLine = `mount C ${destination}`;
    } else if (line.indexOf('cloud_saves') > -1) {
      // skip this line from the config.
      newLine = '';
    }

    newStartupConfig.push(newLine);
  });
