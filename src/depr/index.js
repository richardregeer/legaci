...
function install(fileName, destination) {
  ...
  const { useDefaultConfig, extractor } = program;

  // Extract and install the game package
  if (fileType === fileTypes.SH) {
    shellScriptRunner.run(fileName);
  } else {
    gameExtractor.extract(fileName, destination, extractor);
    gameInstaller.install(fileName, destination, useDefaultConfig);
  }
}
