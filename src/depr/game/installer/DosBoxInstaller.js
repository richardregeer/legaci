...
class DosBoxInstaller {
  ...
  install(fullDestination, useDefaultConfig) {
    let configFilePath = this._getConfigFilePath(fullDestination);
    if (configFilePath === null || useDefaultConfig) {
      const dosBoxconfiguration = this._configurationFactory.createDosBoxConfiguration();
      configFilePath = dosBoxconfiguration.saveConfiguration(fullDestination, './etc/dosbox/dosbox.template.conf');
    }
