...
test.beforeEach((t) => {
  ...
  t.context.extractor = new ZipExtractor(t.context.logger, '.test/tmp', t.context.cli);
});
...
test('Extract should throw an Error if the file to extract does not exist', (t) => {
  ...
  t.throws(() => {
    extractor.extract('/source.zip', '/destination');
  }, Error);
});
...
test('Extract should create the destination directory if it does not exist', (t) => {
  ...
  extractor.extract('.test/install.zip', expectedDestination);
...
test('Extract should move extracted files to given destination', (t) => {
  ...
  extractor.extract('.test/install.zip', '.test/destination');
